...
def get_account(account_number: str) -> Account:
    ...
    return parse(model=AccountItem, event=dynamodb_repository.get_item(table_name=table_name, key=key)).to_dto()


def upsert_account(account: Account) -> Account:
    return parse(model=AccountItem,
                 event=dynamodb_repository.update_item(table_name, AccountItem.from_dto(account))).to_dto()
